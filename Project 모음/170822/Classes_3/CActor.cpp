...
{
	mpTexture = Director::getInstance()->getTextureCache()->addImage("Hero_1.png");
...
{
	mpActor = Sprite::createWithTexture(mpTexture, Rect(0, 0, 64, 64));
...
void CActor::CreateAttackEffect()
{
	mpAttackEffect = Sprite::create("mole_1.png");
	mpAttackEffect->retain();
}

void CActor::RunAttackEffect()
{
	mpAttackEffect->setPosition(mVec);
	mpScene->addChild(mpAttackEffect, 5);
}
...
void CActor::RunAnimation()
{
	...
	auto rep = RepeatForever::create(mpCurAnimate);
	mpActor->runAction(rep);
}
...
void CActor::AnimationFSMSelector()
{
	...
	if (mDirY >= 0 && mDirY >= mDirX && mDirY >= -mDirX)
	{
		//	if (0 != mDir)
		//	{
		this->RunAnimation();
