...
{		
	...
	tVec.x = mpActor->GetVec().x + ((mTouchPosition.x - mpActor->GetVec().x) / mDistance)* mpActor->GetSpeedPower()*dt;
	tVec.y = mpActor->GetVec().y + ((mTouchPosition.y - mpActor->GetVec().y) / mDistance)* mpActor->GetSpeedPower()*dt;

	mpActor->SetVec(tVec);
	mpActor->setPosition(tVec);	
}
