...
void CVirtualPad::TouchesMoved(const std::vector<Touch *> &touches, cocos2d::Event *unused_event)
{
	...
	if (mpJoysticTouched) {
		...
		for (int i = 0; i < touches.size(); i++) {
			...
			if (touch) {
				tap = touch->getLocation();
				theta = atan2f(tap.y, tap.x);

				if (mpSprJoysticPad->getBoundingBox().containsPoint(tap))
				{
					mVecResult.x = tap.x;
					mVecResult.y = tap.y;
				}
				...
				{
					mVecResult.x = mVecStart.x + GetPadScale()*cosf(theta);
					mVecResult.y = mVecStart.y + GetPadScale()*sinf(theta);
				}
