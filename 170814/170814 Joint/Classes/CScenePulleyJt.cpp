...
Scene* CScenePulleyJt::createScene()
{
    auto scene = Scene::create();
    
    auto layer = CScenePulleyJt::create();
...
bool CScenePulleyJt::init()
{
    ...
    auto tBtnClose = MenuItemImage::create(
                                           "CloseNormal.png",
                                           "CloseSelected.png",
                                           CC_CALLBACK_1(CScenePulleyJt::OnClickBtnGoScene, this));
    ...
    {
        this->schedule(schedule_selector(CScenePulleyJt::tick));
    }
...
bool CScenePulleyJt::createBox2dWorld(bool debug)
{
    // 월드 생성 시작 --------------------------------------------------------------------------
...
CScenePulleyJt::~CScenePulleyJt()
{
    delete _world;
    _world = NULL;
}

void CScenePulleyJt::draw(Renderer *renderer, const Mat4& transform, uint32_t flags)
{
    //
    // IMPORTANT:
...
void CScenePulleyJt::onEnter()
{
    ...
    listener->onTouchBegan = CC_CALLBACK_2(CScenePulleyJt::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(CScenePulleyJt::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(CScenePulleyJt::onTouchEnded, this);
...
void CScenePulleyJt::tick(float dt)
{
...
b2Body* CScenePulleyJt::addNewSprite(Point point, Size size, b2BodyType bodytype,
...
b2Body* CScenePulleyJt::getBodyAtTab(Point p)
{
    b2Fixture *fix;
    for (b2Body *b = _world->GetBodyList(); b; b = b->GetNext())
    {
...
bool CScenePulleyJt::onTouchBegan(Touch *touch, Event * event)
{
...
void CScenePulleyJt::onTouchMoved(Touch *touch, Event * event)
{
    Point touchPoint = touch->getLocation();
...
void CScenePulleyJt::onTouchEnded(Touch *touch, Event * event)
{
    if (bDrag) {
...
void CScenePulleyJt::OnClickBtnGoScene(cocos2d::Ref* pSender)
{
