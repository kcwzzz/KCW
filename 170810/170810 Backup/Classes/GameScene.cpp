...
bool GameScene::init()
{
	...
	this->schedule(schedule_selector(GameScene::CreateEnemy_2), 15.0f);
...

{
	...
	for (ti = 0; ti < 10; ti++)
	{
		...
		mpEnemy[ti]->setPosition(Vec2(0-80*ti, 450+20*ti));	//  SetPosition
...
void GameScene::CreateEnemy_1(float f)	//Vec 써서 배열로 나오게 한다.
{
	...
	for (ti = 0; ti < 10; ti++)
	{
		...
		mpEnemy_1[ti]->setPosition(Vec2(80+ 80*ti, 600+120*ti));	//  SetPosition
		mpEnemy_1[ti]->Build();
	}	
}
...
void GameScene::CreateEnemy_2(float f)	//Vec 써서 배열로 나오게 한다.
{
	...
	for (ti = 0; ti < 10; ti++)
	{
		...
		mpEnemy_2[ti]->setPosition(Vec2(480 - 80 * ti, 300 + 80 * ti));	//  SetPosition
		mpEnemy_2[ti]->Build();
	}
}
