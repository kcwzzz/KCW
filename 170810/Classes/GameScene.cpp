...
bool GameScene::init()
{
	...
	this->schedule(schedule_selector(GameScene::ReadyToFire), 1.0f);
...

{
	...
	mpEnemy->setPosition(Vec2(0, 600));	//  SetPosition
	mpEnemy->Build();					//  addChild
}

void GameScene::CreateEnemy_1(int EnemyCount)	//Vec 써서 배열로 나오게 한다.
{
	int ti = 0;
	for (ti = 0; ti > EnemyCount; ti++)
	{
		mpEnemy_1[ti] = new CEnemy_1();				//  CEnemy를 동적 할당
		mpEnemy_1[ti]->Create();					//  Sprite 생성 및 앵커포인트 설정
		mpEnemy_1[ti]->SetScene(this);			//  CEnemy가 GameScene(this)을 타겟하도록 설정
		mpEnemy_1[ti]->setPosition(Vec2(ti * 80, 800));	//  SetPosition
		mpEnemy_1[ti]->Build();					//  addChild
	}
}

void GameScene::ReadyToFire(float f)
{
	mpActor->ReadyToFire(f);
	mpEnemy->ReadyToFire(f);
}
//배경
...
bool GameScene::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *unused_event)
{
	...
	log("%f, %f", mpActor->GetVec().x, mpActor->GetVec().y);
